...
def send_telemetry(client, device_token):
    data = { 
        'id': round(random.uniform(130, 149)),
        'ty': round(random.uniform(1, 4))
    }

    # Randomly decide on the content of 'v': actual value, "NA,NA", or omit it
    v_choice = random.choice(['value'])
    
    if v_choice == 'value':
        data['v'] = f"{random.uniform(18.00, 28.00):.2f},{random.uniform(50.00, 70.00):.2f}"
    # elif v_choice == 'NA':
    #     data['v'] = "NA,NA"
    # If 'v_choice' is 'omit', we do not add 'v' to data
