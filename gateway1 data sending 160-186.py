...
def send_telemetry(client, device_token):
    allowed_ids = [161, 162, 163, 170, 171, 176, 177] + list(range(180, 187))
    id_value = random.choice(allowed_ids)
    ...
    # Randomly decide on the content of 'v': actual value, "No data", or omit it
    v_choice = random.choice(['value'])         #,''       
    if v_choice == 'value':
        data['v'] = f"{random.uniform(24.00, 26.00):.2f},{random.uniform(60.0, 70.00):.2f}"
    # elif v_choice == 'NA':
    #     data['v'] = "NA,NA"
    # If v_choice is 'omit', we simply don't add 'v' to data
